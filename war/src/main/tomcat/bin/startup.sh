...
#-------------------------------------
APPJARPATH='$CLI_REPLACE_APPJARPATH$'
if [[ "$APPJARPATH" =~ ^\$CLI* ]]; then
    APPJARPATH=""
else
    APPJARPATH="$APPJARPATH,"
fi
echo "user-specified additional jar path:$APPJARPATH"
...
SPARK_CLASSPATH="$DOCBASE/WEB-INF/classes/:$DOCBASE/WEB-INF/lib/*:$spark_jars:$SCRIPT_DIR/../lib/*:$crux_jar:-Djava.io.tmpdir=$CATALINA_BASE:/opt/tms/java/pcsaudf.jar$colonSepUdfJarPath:$SPARK_HBASE_JAR"
...
cmd="sh -x /opt/spark/bin/spark-submit $ARG_APP_NAME $ARG_MASTER_MODE $QUEUE_NAME $ARG_POOLCONFIG $ARG_PROPERTIES_FILE $ARG_EXECUTOR_LOGFILE $ARG_CLASS_NAME --jars $APPJARPATH`ls -d -1 $DOCBASE/WEB-INF/lib/* | sed ':a;N;$!ba;s/\n/,/g'`$udfJarPath  --driver-class-path '$SPARK_CLASSPATH' --driver-java-options '$JAVA_OPTS' $core_jar"
