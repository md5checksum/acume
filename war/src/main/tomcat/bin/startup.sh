...
if [[ $app_name -eq -1 ]] && [[ "$prop_loc" != "" ]]; then
    grep_cmd_output=$(cat "$prop_loc" 2>>"$CATALINA_OUT" | grep -v "^[[:space:]]*#" | grep "spark.app.name" )
...
if [[ $master_mode -eq -1 ]] && [[ "$prop_loc" != "" ]]; then
    grep_cmd_output=$(cat "$prop_loc" 2>>"$CATALINA_OUT" |grep -v "^[[:space:]]*#" | grep "spark.master" )
    if [[ ! -z $grep_cmd_output ]]; then
...
if [[ ( $master_mode -eq -1 || "$master_mode" =~ ^yarn* ) && ( $queue_name -eq -1 ) && ("$prop_loc" != "") ]]; then
    grep_cmd_output=$(cat "$prop_loc" 2>>"$CATALINA_OUT" | grep -v "^[[:space:]]*#" |grep "spark.yarn.queue" )
    if [[ ! -z $grep_cmd_output ]]; then
...
if [ ! -z "$prop_loc" ]; then
    grep_poolfile=$(cat "$prop_loc" 2>>"$CATALINA_OUT" |grep -v "^[[:space:]]*#" | grep "spark.scheduler.allocation.file" )
    ARG_POOLCONFIG=" --conf "
