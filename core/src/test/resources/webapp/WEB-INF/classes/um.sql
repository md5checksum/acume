...
create table Filter (id bigserial, description varchar(255), filterMap bytea, moduleId varchar(255) not null, name varchar(255), responseFilters bytea, searchRequest bytea, version integer not null, userName varchar(255), primary key (id));
...
alter table Filter add constraint FK7D6DB79844F54FE3 foreign key (userName) references UserInfo;
...
#VER 3.7.7.3
alter table Filter drop constraint FK7D6DB79844F54FE3;
alter table Filter add constraint ON_DELETE_CONST foreign key (USERNAME) references UserInfo ON DELETE CASCADE;
#VER 4.1
