...
import com.guavus.qb.services.DummyQBService
...
import com.guavus.acume.core.TMOContext
...
class TMOAppConfig extends AcumeAppConfigTrait {
  ...
  override def acumeContext() : AcumeContextTrait = {
    AcumeContextTrait.acumeContext = Some(new TMOContext(AcumeContextTrait.acumeConf))
  ...
  override def queryBuilderService(acumeContext : AcumeContextTrait) : Seq[IQueryBuilderService] = {
    ...
    List(new DummyQBService(new AcumeDataSourceSchema(acumeContext), new QBConf()))
  }
