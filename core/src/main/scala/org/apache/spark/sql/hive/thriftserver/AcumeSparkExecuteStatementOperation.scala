...
import org.apache.spark.sql.catalyst.expressions.MetaField
...
private[hive] class AcumeSparkExecuteStatementOperation(
    ...
    if (result == null || result.queryExecution.analyzed.output.size == 0) {
      ...
      val schema = result.queryExecution.analyzed.output.map { attr =>
        new FieldSchema(attr.name,
          ...
          "",
          attr.metadata.getOrNull(MetaField.DEFAULT))
      }
  ...
  override def runInternal(): Unit = {
    ...
    AcumeHiveThriftServer2.listener.onStatementStart(
      statementId,
    ...
    try {
      ...
      AcumeHiveThriftServer2.listener.onStatementParsed(statementId, result.queryExecution.toString())
      iter = {
      ...
      case e: Throwable =>
        ...
        setState(OperationState.ERROR)
        AcumeHiveThriftServer2.listener.onStatementError(
          statementId, e.getMessage, e.getStackTraceString)
        throw new HiveSQLException(e.toString)
    }
    setState(OperationState.FINISHED)
    AcumeHiveThriftServer2.listener.onStatementFinish(statementId)
  }
