  ...
  private def getVariableRetentionMap: SortedMap[Long, Int] = {
   ...
   def loadData(businessCube: Cube, levelTimestamp: LevelTimestamp, dTableName: DimensionTable): Tuple2[SchemaRDD, String] = {
    ...
    this.synchronized {
    	  ...
    	  val dimensionSetRdd = diskUtility.loadDimensionSet(keyMap, businessCube, Utility.floorFromGranularity(metaData.get(DataLoadedMetadata.dimensionSetEndTime).toLong, businessCube.baseGran.getGranularity), endTime)
    	  val fullRdd = AcumeStarSchemaTreeCache.generateId(dimensionSetRdd, dTableName, acumeCacheContext.cacheSqlContext)
    	  fullRdd.cache
    	  dTableName.Modify
...
object AcumeStarSchemaTreeCache {
  ...
  def generateId(drdd: SchemaRDD, dtable : DimensionTable, sqlContext : SQLContext): SchemaRDD = {
