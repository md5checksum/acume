...
//    conf123.set(ConfConstants.evictionpolicy, "com.guavus.acume.cache.eviction.VREvictionPolicy")
    val output = cntxt.acql("select hit_count from \"searchEgressPeerCube\" where ts >=1404723600 and ts <1404727200").schemaRDD.collect
    output.map(_.mkString(",")).map(print)
